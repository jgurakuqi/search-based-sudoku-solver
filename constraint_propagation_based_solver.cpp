...
vector<vector<int>> solveSudoku(vector<vector<int>> sudokuField)
{
    ...
    vector<vector<vector<int>>> sudokuCellsDomains = createSudokuDomain(sudokuField);
    return solveSudokuI(sudokuField, sudokuCellsDomains);
}
...
vector<vector<int>> solveSudokuI(vector<vector<int>> sudokuField, vector<vector<vector<int>>> sudokuCellsDomains)
{
    ...
    while (!sudokuCellsDomains[i][j].empty() && result != nullptr)
    {
